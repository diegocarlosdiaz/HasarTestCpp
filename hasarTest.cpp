...
// Complete la siguiente función climbingLeaderboard. NOTA: Se deberá elegir el tipo y
//estructura de datos usar.
int* climbingLeaderboard(int scores_count, int* scores, int alice_count, int* alice, int* result_count)
{
    int* ranking = new int[scores_count+1];
    int pos = 1;
    ...
    int* aliceRanking = new int[alice_count]{};
    int cant = 0;
    ...
    for (int i =0; i< alice_count; i++){
        if(aliceRanking[i] != aliceRanking[i+1]){
            for(int j =0; j<cant;j++){
                result_count[j] = aliceRanking[i];
            }
        }
    }
...
int main()
{
    ...
    if (scores_count_endptr == scores_count_str || *scores_count_endptr != '\0')
    {
        exit(EXIT_FAILURE);
    }
